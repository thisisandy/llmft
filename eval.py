...
from eval_utils import _select_subset_by_idx, create_few_shot_context
...
from options import DataTrainingArguments, InContextLearningArguments, ModelArguments
...
def _load_model(model_args, device=None):
    ...
    #     config.pad_token_id = tokenizer.eos_token_id
        ...
        "gpt-neox" in model_args.model_name_or_path
        or "pythia" in model_args.model_name_or_path
        or "RedPajama-INCITE" in model_args.model_name_or_path
    ):
...
def _add_args_to_results(args, results):
    ...
    results["task_description"] = (
        args.task_description if args.task_description is not None else " "
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, training_args, in_context_args = (
    ...
    target_tokens = [t.strip() for t in in_context_args.target_tokens.split(",")]
    ...
    token_id_to_label_id = {tidx: lidx for lidx, tidx in enumerate(target_tokens_ids)}
    ...
    # tokenize context
    result = tokenizer(
        context, padding=padding, max_length=max_seq_length, truncation=False
    )
    ...
    def preprocess_function(examples):
        ...
        pattern_examples = [
            pattern.format(
                ...
                text2=(
                    examples[sentence2_key][idx] if sentence2_key is not None else None
                ),
            )
        ...
        # Get tokens
        result["input_tokens"] = [
            tokenizer.convert_ids_to_tokens(ids) for ids in result["input_ids"]
        ]

        # Decode input
        result["input_text"] = [tokenizer.decode(ids) for ids in result["input_ids"]]

        # Replace labels by target tokens indices when using lm_head
        result["label"] = [target_tokens_ids[l] for l in examples["label"]]
        result["label_text"] = [
            id_to_target_token[l] if l != -1 else "unlabeled" for l in examples["label"]
        ]
    ...
    if training_args.do_eval:
        ...
        eval_dataset = raw_datasets[
            (
                "validation_matched"
                if data_args.task_name in ["mnli", "mnli-original"]
                else "validation"
            )
        ]
        ...
        if (
            in_context_args.target_tokens is not None
            and not in_context_args.target_tokens_logits_only
        ):
            new_features = eval_dataset.features.copy()
            names = [f"{idx}" for idx in np.arange(len(tokenizer))]
            new_features["label"] = ClassLabel(names=names, num_classes=len(tokenizer))
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        class_logits = [
            [logits[target_tokens_ids[0]], logits[target_tokens_ids[1]]]
    ...
    if training_args.do_eval:
        ...
        eval_task_names += [
            task_name for task_name in additional_evaluation_datasets.keys()
        ]
        eval_datasets = [eval_dataset]
        eval_datasets += [
            dataset for _, dataset in additional_evaluation_datasets.items()
        ]
        ...
        if trainer.is_world_process_zero():
            ...
            if "llama" in model_args.model_name_or_path:
                ...
                file_name = (
                    f"{model_args.model_name_or_path.replace('/', '-')}"
                    + f"_{data_args.task_name}"
                    + f"_{data_args.eval_task_name}"
                )
